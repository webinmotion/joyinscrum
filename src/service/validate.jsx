...
export function validateLinkAddress(href, onComplete) {
    if (!href) {
        onComplete('link address is a required field');
    } else if (!LINK_HREF_REGEX.test(href)) {
        onComplete('link address should start with a protocol followed by an origin');
    } else {
        onComplete();
    }
}