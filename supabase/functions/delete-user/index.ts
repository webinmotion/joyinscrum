...
/**
 * Deleting users in Supabase is a straightforward process, but it requires understanding the 
 * implications on user sessions and related objects. When you delete a user from the auth.users table, 
 * their session remains active until the JWT expires. 
 * 
 * To revoke access immediately, implement a Row Level Security policy.
 */
...
serve(async (request) => {
  ...
  try {
    ...
    return new Response(JSON.stringify(deletion_data, null, 2), {
